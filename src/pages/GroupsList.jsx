...
import {
 collection, addDoc, query, where,
  onSnapshot, serverTimestamp, doc, updateDoc, arrayUnion, deleteDoc, getDocs,
} from "firebase/firestore";
import { useNavigate } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import { Users, Plus, Copy, Check, ArrowLeft, Trash2 } from "lucide-react";
...
export default function GroupsList() {
  ...
  // ── Listen to all groups where current user is a member ──
  useEffect(() => {
    if (!user) return;
    const q = query(
      ...
      <div style={{ maxWidth: "480px", margin: "0 auto" }}>
        ...
        <AnimatePresence>
          {showCreate && (
            ...
            >
              <p style={{ color: "white", fontWeight: "600", fontSize: "16px", marginBottom: "16px" }}>Create a new group</p>
              <input
                type="text"
        ...
        {groups.length === 0 ? (
          <div style={{ ...glassCard, padding: "48px 24px", textAlign: "center" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "13px" }}>Create one or join via an invite link</p>
          </div>
        ) : (
            ...
            {groups.map((group) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ color: "white", fontWeight: "600", fontSize: "15px", margin: 0, marginBottom: "4px" }}>{group.name}</p>
                ...
                {/* Copy link button */}
                <button
                  onClick={(e) => { e.stopPropagation(); copyLink(group.id); }}
