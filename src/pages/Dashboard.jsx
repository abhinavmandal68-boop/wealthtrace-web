...
export default function Dashboard({ user }) {
  ...
  const detectBank = (headerLine = "") => {
    ...
    if (
      ...
      h.includes("ref no./cheque")
    )
      return "SBI";

    if (
      h.includes("withdrawal amt") ||
      h.includes("deposit amt")
    )
      return "HDFC";

    if (
      h.includes("transaction remarks") ||
      h.includes("dr/cr")
    )
      return "ICICI";
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const headers = parseCSVLine(headerRaw).map((h) =>
          h.toLowerCase()
        );

        const col = (kws) =>
          headers.findIndex((h) =>
            kws.some((k) => h.includes(k))
          );
        ...
        lines.slice(headerIdx + 1).forEach((line, idx) => {
          ...
          if (
            skipWords.some((k) => ll.startsWith(k))
          )
            return;
          ...
          if (
            !txnDateParsed ||
            isNaN(txnDateParsed.getTime())
          ) {
            if (rawDate.length > 0) {
              errors.push(
                `Row ${idx + 2}: cannot parse date "${rawDate}"`
              );
            }
    ...
    >
        ...
        * {
        ...
        .tab-btn.active {
          background: linear-gradient(
            135deg,
            rgba(120,40,200,0.75),
            rgba(40,120,200,0.75)
          );
          color: white;
          box-shadow:
            0 4px 16px rgba(120,40,200,0.3);
        }
        ...
        .action-btn {
          ...
          box-shadow:
            0 6px 24px rgba(120,40,200,0.4);
        }

        .action-btn:hover {
          transform: translateY(-2px);
          box-shadow:
            0 10px 32px rgba(120,40,200,0.5);
        }
        ...
        /* =====================================================
           ...
           The navigation NEVER becomes wider than the screen.
           ===================================================== */
        ...
        @media (max-width: 768px) {
          ...
          .dashboard-page .tx-row > div:last-child {
            min-width: 0;
          }

          .dashboard-page
          [style*="position:fixed"][style*="bottom:32px"] {
      ...
      <div
        style={{
          ...
          height: "400px",
          background:
            "radial-gradient(circle,rgba(120,40,200,0.15) 0%,transparent 70%)",
          borderRadius: "50%",
          pointerEvents: "none",
          animation:
      ...
      <div
        style={{
          ...
          height: "400px",
          background:
            "radial-gradient(circle,rgba(40,120,200,0.15) 0%,transparent 70%)",
          borderRadius: "50%",
          pointerEvents: "none",
          animation:
            "float 14s ease-in-out infinite 2s",
        }}
      />
