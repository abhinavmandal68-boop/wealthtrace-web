...
import { Trash2, LogOut, Upload, TrendingUp, TrendingDown } from "lucide-react";
...
export default function Dashboard() {
  ...
  const parseDate = (raw) => {
    ...
    if (dmyS)     return new Date(2000 + +dmyS[3], +dmyS[2]-1, +dmyS[1]);
    const hdfc    = s.match(/^(\d{1,2})-([A-Za-z]{3})-(\d{4})$/);
    if (hdfc)     return new Date(`${hdfc[2]} ${hdfc[1]} ${hdfc[3]}`);
  ...
  const detectCategory = (desc = "") => {
    ...
    if (d.includes("uber")||d.includes("ola")||d.includes("petrol")||d.includes("irctc")||d.includes("flight")) return "Transport";
    ...
    reader.onload = async (event) => {
      try {
        ...
        lines.slice(headerIdx + 1).forEach((line, idx) => {
          ...
          batch.set(ref, {
            amount: finalAmount, type: transType,
            category: detectCategory(rawDesc), desc: rawDesc,
            userId: user.uid,
            createdAt: { seconds: Math.floor(txnDateParsed.getTime()/1000), nanoseconds: 0 },
            bankDate: rawDate, bank, imported: true,
          });
          count++;
        });
  ...
  const barData = [...filtered].reverse().reduce((acc, t) => {
    const date = t.createdAt?.seconds
      ? new Date(t.createdAt.seconds * 1000).toLocaleDateString("en-IN", { day:"2-digit", month:"short" })
      : "Syncing";
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg, #0f0c29 0%, #302b63 50%, #24243e 100%)", fontFamily:"'Outfit', sans-serif", color:"white", position:"relative" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .tab-btn { background:none; border:none; cursor:pointer; font-family:'Outfit',sans-serif; font-size:14px; font-weight:600; letter-spacing:0.03em; padding:10px 22px; border-radius:50px; transition:all 0.25s ease; color:rgba(255,255,255,0.45); }
        .tab-btn.active { background:linear-gradient(135deg,rgba(120,40,200,0.75),rgba(40,120,200,0.75)); color:white; box-shadow:0 4px 16px rgba(120,40,200,0.3); }
        ...
        .ghost-btn { background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.10); color:rgba(255,255,255,0.5); border-radius:10px; padding:8px 14px; font-family:'Outfit',sans-serif; font-size:13px; font-weight:500; cursor:pointer; transition:all 0.2s; display:flex; align-items:center; gap:6px; }
        .ghost-btn:hover { background:rgba(255,255,255,0.10); color:white; border-color:rgba(255,255,255,0.2); }

        .danger-btn { background:rgba(239,68,68,0.10); border:1px solid rgba(239,68,68,0.2); color:rgba(239,68,68,0.7); border-radius:10px; padding:8px 14px; font-family:'Outfit',sans-serif; font-size:13px; font-weight:500; cursor:pointer; transition:all 0.2s; display:flex; align-items:center; gap:6px; }
        ...
        .field-input { width:100%; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.10); border-radius:12px; padding:12px 16px; color:white; font-family:'Outfit',sans-serif; font-size:15px; font-weight:500; outline:none; transition:border-color 0.2s; }
          ...
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"24px" }}>
              ...
              <select value={filterMonth} onChange={e => setFilterMonth(e.target.value)}
                style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.10)", borderRadius:"10px", color:"white", padding:"8px 14px", fontSize:"13px", fontWeight:"500", outline:"none", cursor:"pointer", fontFamily:"'Outfit',sans-serif" }}>
                ...
                <div style={{ ...glassCard, padding:"28px" }}>
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={barData} barGap={4}>
                      ...
                      <Tooltip contentStyle={{ background:"rgba(15,12,41,0.95)", border:"1px solid rgba(255,255,255,0.10)", borderRadius:"12px", fontFamily:"Outfit", fontSize:"13px" }} labelStyle={{ color:"rgba(255,255,255,0.6)", marginBottom:"4px" }} formatter={(val, name) => [`Rs. ${val.toLocaleString("en-IN")}`, name === "income" ? "Income" : "Expense"]} />
                  ...
                  {pieData.length > 0 ? (
                    <div style={{ display:"flex", gap:"16px", alignItems:"center" }}>
                      <PieChart width={160} height={160}>
                        ...
                        <Tooltip contentStyle={{ background:"rgba(15,12,41,0.95)", border:"1px solid rgba(255,255,255,0.10)", borderRadius:"12px", fontFamily:"Outfit", fontSize:"13px" }} formatter={val => [`Rs. ${val.toLocaleString("en-IN")}`, ""]} />
                      </PieChart>
                  ...
                  {filtered.slice(0, 5).map(t => (
                    <div key={t.id} className="tx-row">
                      <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                        <div style={{ width:36, height:36, borderRadius:"10px", background: t.type==="income" ? "rgba(52,211,153,0.10)" : "rgba(248,113,113,0.10)", display:"flex", alignItems:"center", justifyContent:"center" }}>
                        ...
                        <div>
                          ...
                          <p style={{ fontSize:"11px", color:"rgba(255,255,255,0.30)", margin:0, marginTop:"2px" }}>{t.desc || formatTxDate(t)}</p>
                        </div>
                      </div>
        ...
        {activeTab === "add" && (
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }} style={{ maxWidth:"520px", margin:"0 auto" }}>
            <div style={{ ...glassCard, padding:"36px", position:"relative", overflow:"hidden" }}>
              ...
              {/* CSV Import */}
              <div style={{ marginTop:"24px", padding:"20px", background:"rgba(255,255,255,0.03)", border:"1px dashed rgba(255,255,255,0.10)", borderRadius:"14px" }}>
                      ...
                      <motion.div key={t.id} layout
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                          {selectMode ? (
                            ...
                            <div style={{ width:38, height:38, borderRadius:"10px", background: t.type==="income" ? "rgba(52,211,153,0.10)" : "rgba(248,113,113,0.10)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
